...
<template>
  <Menu id="user_info_panel" ref="menu" :popup="true" class="p-sm">
    <template #start>
      <div class="flex flex-rol flex-justify-between flex-items-center">
        ...
        <div class="text-right">
          ...
          <div class="text-gray-500 text-sm">QQ {{ runtimeData.userInfo.value.main.user_id }}</div>
        </div>
      </div>
      <Divider />
      <div
        v-if="runtimeData.userInfo.value.more?.longNick"
        class="flex flex-rol flex-justify-between flex-items-center min-w-60"
      >
        <div class="text-gray-500 text-sm mr-4">签名</div>
        <div class="text-right text-sm">{{ runtimeData.userInfo.value.more?.longNick }}</div>
      </div>
    </template>
    <template #end>
      <Divider />
      <div class="flex flex-row flex-justify-between flex-items-center gap-sm w-full">
